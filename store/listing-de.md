# Play-Store-Eintrag — Deutsch (de-DE)

## App-Name (max. 30 Zeichen)
`Parkplatz` *(oder mit Keyword: `Parkplatz – Auto Puzzle`, 24 Zeichen)*

## Kurzbeschreibung (max. 80 Zeichen)
`Befreie das rote Auto aus dem Stau – 100 knifflige Schiebe-Rätsel!`

## Vollständige Beschreibung (max. 4000 Zeichen)

Parkplatz ist ein entspannendes und zugleich forderndes Schiebe-Puzzle im Stil
von „Rush Hour": Der Parkplatz ist zugeparkt und dein rotes Auto muss raus!
Verschiebe Autos, Busse, LKWs und Motorräder geschickt zur Seite, bis der Weg
zur Ausfahrt frei ist.

100 handgeprüfte Level in vier Schwierigkeitsstufen – von leicht bis sehr
schwer, aber keines davon langweilig. Für jedes Level ist die minimale Zugzahl
bekannt: Schaffst du es mit den wenigsten Zügen, gibt es drei Sterne!

★ FUNKTIONEN
• 100 einzigartige, garantiert lösbare Rätsel
• Vier Schwierigkeitsstufen mit sanft steigender Herausforderung
• 3-Sterne-Bewertung – hol dir das perfekte Ergebnis
• Level-Editor: Baue eigene Rätsel mit Live-Lösbarkeitsprüfung
• Tägliches Rätsel – jeden Tag ein neues, für alle gleich
• Zufallsrätsel in jeder Schwierigkeit
• Tipp-Funktion, Rückgängig und Neustart
• Drei Designs: Tag, Nacht und Neon
• Farbenblind-Modus und reduzierte Animationen
• Maus-, Touch- und Tastatursteuerung

★ KOMPLETT OFFLINE & FAIR
• Keine Werbung, keine In-App-Käufe, keine Tracker
• Funktioniert ohne Internetverbindung
• Dein Fortschritt bleibt auf deinem Gerät

★ 15 SPRACHEN
Deutsch, Englisch, Spanisch, Französisch, Italienisch, Portugiesisch,
Niederländisch, Polnisch, Chinesisch (vereinfacht & traditionell), Japanisch,
Arabisch, Koreanisch, Russisch und Türkisch.

Mach den Parkplatz frei – ein Zug nach dem anderen!
