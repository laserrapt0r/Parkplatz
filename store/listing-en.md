# Play Store listing — English (en-US)

## App name (max. 30 chars)
`Parkplatz` *(or with keyword: `Parkplatz — Car Puzzle`, 22 chars)*

## Short description (max. 80 chars)
`Get the red car out of the traffic jam — 100 clever sliding puzzles!`

## Full description (max. 4000 chars)

Parkplatz is a relaxing yet brain-teasing sliding-block puzzle in the spirit of
"Rush Hour": the car park is jammed and your red car needs to get out! Slide
cars, buses, trucks and motorcycles out of the way until the path to the exit is
clear.

100 hand-verified levels across five difficulty tiers — from easy to extreme,
and never dull. Every level's minimum number of moves is known: match it and you
earn all three stars!

★ FEATURES
• 100 unique, guaranteed-solvable puzzles
• Five difficulty tiers (up to “Extreme”) with a smoothly rising challenge
• 3-star rating — go for the perfect run
• Level editor: build your own puzzles with a live solvability check
• Daily puzzle — a fresh one every day, the same for everyone
• Random puzzles at any difficulty
• Hint, Undo and Restart
• Three themes: Day, Night and Neon
• Colorblind mode and reduced-motion option
• Mouse, touch and keyboard controls

★ FULLY OFFLINE & FAIR
• No ads, no in-app purchases, no trackers
• Works without an internet connection
• Your progress stays on your device

★ 15 LANGUAGES
German, English, Spanish, French, Italian, Portuguese, Dutch, Polish, Chinese
(Simplified & Traditional), Japanese, Arabic, Korean, Russian and Turkish.

Clear the car park — one move at a time!
