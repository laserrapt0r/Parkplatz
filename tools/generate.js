...
// ---------- generate pool ----------
const MIN_MOVES = 6;
const COMP_CAP = 200000;     // skip piece sets whose graph is larger than this
const PER_COMPONENT = 12;    // varied puzzles harvested from each component
const TARGET_POOL = 9000;
...
while (pool.length < TARGET_POOL && (Date.now() - t0) < TIME_BUDGET_MS) {
  seeds++;
  const fillers = 8 + rint(6); // 8..13 extra fillers (denser -> deeper graphs)
...
// 5 difficulty tiers with fixed counts (fewer easy, more hard, + Extrem).
const TIER_DEFS = [['leicht', 15], ['mittel', 25], ['schwer', 25], ['sehr_schwer', 20], ['extrem', 15]];
const tierByIndex = [];
TIER_DEFS.forEach(([name, cnt]) => { for (let k = 0; k < cnt; k++) tierByIndex.push(name); });
...
let off = 0;
for (const [name, cnt] of TIER_DEFS) {
  const slice = puzzles.slice(off, off + cnt); off += cnt;
  ...
  console.error(`${name}: n=${slice.length} min=${Math.min(...mm)} max=${Math.max(...mm)} avg=${(mm.reduce((a, b) => a + b, 0) / mm.length).toFixed(1)} | distinct piece-sets: ${setsT.size}/${cnt}`);
}
console.error(`TOTAL distinct piece-sets across 100 levels: ${allSets.size}`);
